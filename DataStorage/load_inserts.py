...
DBname = "postgres"
DBuser = "postgres"
DBpwd = "psql908243"   # insert your postgres db password here
...
def load_copy_from(conn, filepath):
    with conn.cursor() as cursor:
        print(f"Using COPY FROM to load: {filepath}")
        start = time.perf_counter()
        ...
        conn.commit()
        elapsed = time.perf_counter() - start
        print(f'Finished COPY FROM. Elapsed Time: {elapsed:0.4f} seconds')
